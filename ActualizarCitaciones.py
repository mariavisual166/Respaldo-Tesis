...
def Actualizar():
	...
	if((int(time.strftime("%d"))==6 or int(time.strftime("%d"))==10 or int(time.strftime("%d"))==20) and int(time.strftime("%M"))==27 and int(time.strftime("%S"))==1 and int(time.strftime("%H"))==13 ):
		
		ListaDeNombresDelSistema=[]
		ListaDePirmerApellidoDelSistema=[]
		ListaDePublicaciones=[]
		ListaAux=[]
		sqlquery7 = "SELECT p.TituloPublicacion,d.PirmerNombre,d.PirmerApellido FROM Docente d ,Publicacion p ,DocenteTienePublicacion t where t.IdPublicacion=P.id and t.CedulaPersona=d.Cedula ;"
